    ...
    m_contentChanged(false)
{
    ...
    connect(m_ui->actionAbout_QtMindMap, SIGNAL(triggered()),
            this, SLOT(about()));
...
void MainWindow::setupMainToolbar()
{
    ...
    connect(m_addNode, SIGNAL(triggered()), m_graphicsView->graphLogic(),
            SLOT(insertNode()));

    m_delNode = new QAction(tr("Del node (del)"), this);
    connect(m_delNode, SIGNAL(triggered()), m_graphicsView->graphLogic(),
            SLOT(removeNode()));

    m_editNode = new QAction(tr("Edit node (F2, dubclick)"), this);
    connect(m_editNode, SIGNAL(triggered()), m_graphicsView->graphLogic(),
            SLOT(nodeEdited()));

    m_scaleUpNode = new QAction(tr("ScaleUp Node (Ctrl +)"), this);
    connect(m_scaleUpNode, SIGNAL(triggered()), m_graphicsView->graphLogic(),
            SLOT(scaleUp()));
    m_scaleDownNode = new QAction(tr("ScaleDown Node (Ctrl -)"), this);
    connect(m_scaleDownNode, SIGNAL(triggered()), m_graphicsView->graphLogic(),
            SLOT(scaleDown()));

    m_nodeColor = new QAction(tr("Node color (c)"), this);
    connect(m_nodeColor, SIGNAL(triggered()), m_graphicsView->graphLogic(),
            SLOT(nodeColor()));

    m_nodeTextColor = new QAction(tr("Node textcolor (t)"), this);
    connect(m_nodeTextColor, SIGNAL(triggered()), m_graphicsView->graphLogic(),
            SLOT(nodeTextColor()));

    m_addEdge = new QAction(tr("Add edge (a)"), this);
    connect(m_addEdge, SIGNAL(triggered()), m_graphicsView->graphLogic(),
            SLOT(addEdge()));

    m_delEdge = new QAction(tr("Del edge (d)"), this);
    connect(m_delEdge, SIGNAL(triggered()), m_graphicsView->graphLogic(),
            SLOT(removeEdge()));
    ...
    connect(m_esc, SIGNAL(triggered()), m_graphicsView->graphLogic(),
            SLOT(nodeLostFocus()));

    m_hintMode = new QAction(tr("Hint mode (f)"), this);
    connect(m_hintMode, SIGNAL(triggered()), m_graphicsView->graphLogic(),
            SLOT(hintMode()));
...
void MainWindow::setupEditToolbar()
{
    ...
    connect(m_mainToolbar, SIGNAL(triggered()),
    ...
    connect(m_iconToolbar, SIGNAL(triggered()),
