...
namespace SyntopiaCore {
	namespace GLEngine {
		...
		EngineWidget::EngineWidget(QMainWindow* mainWindow, QWidget* parent) 
			: QOpenGLWidget(parent), mainWindow(mainWindow) 
		{
			showDepth = false;
			disabled = false;
			updatePerspective();
		...
		void EngineWidget::setupFragmentShader() {
			...
			bool s = shaderProgram->addShaderFromSourceCode(QOpenGLShader::Vertex,
				"varying vec2 coord;\n"
				"void main(void)\n"
				"{\n"
			...
			s = shaderProgram->addShaderFromSourceCode(QOpenGLShader::Fragment,
